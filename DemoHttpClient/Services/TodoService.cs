...
namespace DemoHttpClient.Services
{
    public class TodoService : ITodoService
    {
        ...
        public async Task<Todo> CreateTodoAsync(Todo task)
        {
            ...
            var httpResponse = await _client.PostAsync(BaseUrl, new StringContent(content));
        ...
        {
            var content = JsonConvert.SerializeObject(task);
            var httpResponse = await _client.PutAsync($"{BaseUrl}{task.Id}", new StringContent(content));
            ...
            var updatedTask = JsonConvert.DeserializeObject<Todo>(await httpResponse.Content.ReadAsStringAsync());
            return updatedTask;
        }
